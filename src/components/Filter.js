...
const Filter = () => {
  const {
    handleFilter,
    filters: { search, colors, price, maxPrice, category },
    allCopyProducts,
    handleClearFilter,
  } = useProduct();
        ...
        {categories?.map((curCat, i) => {
          return (
            <button
              type="button"
              key={i}
              className={category === curCat ? "cat-btn active" : "cat-btn"}
              name="category"
              value={curCat}
      ...
      <div className="category">
        ...
        <select name="company" onClick={handleFilter}>
          {companies?.map((curCom, i) => {
            return (
              <option key={i} value={curCom} name="company">
                {curCom}
              </option>
            );
          })}
          ...
          {colorsData?.map((curColor, i) => {
            ...
            return (
              <button
                ...
                className="color-btn flex items-center justify-center"
                name="colors"
                value={curColor}
                onClick={handleFilter}
              >
                {colors === curColor && <GoCheck style={{ color: "white" }} />}
              </button>
            );
          })}
        </div>
